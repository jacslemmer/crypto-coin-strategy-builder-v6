// scripts/step1-fetch.js
// Step 1 helper: Fetch top coins via V6 module and write CSV+JSON to Desktop/Crypto Strategy V6
...
async function main() {
  ...
  const baseDir = join(homedir(), 'Desktop', 'Crypto Strategy V6', 'data', 'crypto-pairs');
  ensureDir(baseDir);
  const timestamp = getTimestamp();
  const csvPath = join(baseDir, `crypto_pairs_${timestamp}.csv`);
  const jsonPath = join(baseDir, `crypto_pairs_${timestamp}.json`);

  // Use V6 defaults (top 100, single page, delays respected inside module as needed)
...
main().catch(err => {
  console.error('Step 1 failed:', err?.message || err);
  process.exit(1);
});
